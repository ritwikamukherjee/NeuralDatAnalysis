...
switch opt.method,
    ...
    case {'simpletiming','simple'},
        dt = diff(t,[],2);
%         dt = cat(2, repmat(2*opt.interburstdur, [size(t,1) 1]), ...
%             dt, ...
%             repmat(2*opt.interburstdur, [size(t,1) 1]));
%         
        for ch = 1:nchan,
            burststart1 = find((dt(ch,1:end-1) > opt.interburstdur) & (dt(ch,2:end) <= opt.interburstdur)) + 1;
            burstend1 = find((dt(ch,1:end-1) <= opt.interburstdur) & (dt(ch,2:end) > opt.interburstdur));
