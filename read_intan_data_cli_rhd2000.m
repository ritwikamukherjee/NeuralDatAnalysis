...
if (data_present)
    ...
    if (notch_filter_frequency > 0) & notch_filt
...
if num_amplifier_channels>0
	...
	if data_present
		amp.data=amplifier_data;
		amp.t=t_amplifier;
	end
	amp.spike_triggers=spike_triggers;
else
	amp=[];
end

if num_aux_input_channels>0
	aux_input.channels=aux_input_channels;
	if data_present
		aux_input.data=aux_input_data;
		aux_input.t=t_aux_input;
	end
else
	aux_input=[];
end
...
if num_supply_voltage_channels>0
	supply_voltage.channels=supply_voltage_channels;
	if data_present
		supply_voltage.data=supply_voltage_data;
		supply_voltage.t=t_supply_voltage;
	end
else
	supply_voltage=[];
end
...
if num_board_dig_in_channels>0
	dig_in.channels=board_dig_in_channels;
	if data_present
...
if num_board_dig_out_channels>0
	dig_out.channels=board_dig_out_channels;
	if data_present
...
if num_temp_sensor_channels>0
	if data_present
		temp_sensor.data=temp_sensory_data;
		temp_sensor.t=t_temp_sensor;
	end
else
	temp_sensor=[];
end
